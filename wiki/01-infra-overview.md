# DevOps System — Overview & Onboarding Guide

> **Audience:** New DevOps / SRE engineers joining {YourCompany}.
...
11. [Self-Hosted GitHub Runner ({GITHUB_RUNNER_SERVER})](#11-self-hosted-github-runner)
...
| Repository | Purpose |
|---|---|
| `github.com/{your-github-org}/terraform` | All AWS infrastructure (VPCs, EKS, RDS, S3, IAM, etc.) |
| `github.com/{your-github-org}/terraform-eks-module` | Reusable Terraform module to provision an EKS cluster with standard add-ons |
| `github.com/{your-github-org}/k8s-manifest` | All Kubernetes manifests — Helm, ArgoCD, Kustomize |
| `github.com/{your-github-org}/cloudflare-{yourcompany}-tf` | Cloudflare DNS zones and records for all {YourCompany} domains |
| `github.com/{your-github-org}/route53-{your-org}-com-tf` | AWS Route 53 DNS records for `{your-org}.com` |
| `github.com/{your-github-org}/route53-{yourcompany}-ai-tf` | AWS Route 53 DNS records for `{yourcompany}.ai` |
| `github.com/{your-github-org}/cloudflare-zero-trust-tf` | Cloudflare Zero Trust configuration (access policies, tunnels) |
...
```
[Argo Deployer] Update backend to a1b2c3d-uat on uat
```
...
> Always read the existing `additionalManifests[0].spec.data[0].remoteRef.key` from the values file to get the exact secret name — do not guess. DEV and UAT may use different names (e.g. `{SECRET_PREFIX_DEV}edu-backend` vs `{SECRET_PREFIX_UAT}edu-backend`).
...
Deployed via Helmfile (`helm/kube-prometheus-stack/`) on every cluster.
...
Centralized log collection runs on the **DevOps server ({DEVOPS_SERVER_1})**. Graylog aggregates logs from all environments into a single UI — logs from each cluster are tagged with their environment label via Filebeat.

- **OpenSearch** — runs as a systemd service. Stores and indexes all container logs.
- **Graylog** — runs as a systemd service. Web UI for log search and alerting, backed by OpenSearch. Exposed via UAT cluster ingress → `raw/uat/logging/ingress.yaml`.
- **Filebeat** — deployed in each EKS cluster via Helmfile (`helm/filebeat/`, version `~8.5.1`). Collects container logs and ships them to Graylog:
...
- **Elastic APM Server** — port `8200`, receives trace data from instrumented applications.
...
|---|---|---|---|---|
| **Elasticsearch** v8.19.4 | `elasticsearch` | 9200 | — | APM trace and log storage |
| **Kibana** | `kibana` | 5601 | `{TRACING_URL}` | APM UI — single URL for all environments |
| **Elastic APM Server** | `apm-server` | 8200 | — | Receives traces from instrumented apps |

### Managing Services

```bash
# Systemd services on {DEVOPS_SERVER_1} (OpenSearch, Graylog)
sudo systemctl status opensearch
sudo systemctl restart graylog-server

# Systemd services on {APM_SERVER} (APM stack)
...
**Server:** `{GITHUB_RUNNER_SERVER}` (separate from the DevOps server)

GitHub Actions CI/CD jobs run on a self-hosted runner, not GitHub-hosted machines. Runner configuration (`/data/github-runner/docker-compose.yml`):
...
The runner container has access to the host Docker socket (`/var/run/docker.sock`) for Docker-in-Docker builds, and mounts the host's Docker config for ECR authentication — the host must have valid ECR credentials.
...
> `staging` and `stag` are used interchangeably across file names and resource names. Dev and UAT share the same EKS cluster (`{uat-cluster-name}`) but are isolated by Kubernetes namespace.
